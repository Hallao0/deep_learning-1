...
def setup_autoencoder_with_pretraining(training_info, user_param): 
	...
	log10_learning_rate = numpy.random.uniform(-1, -2)
	learning_rate = pow(10, log10_learning_rate)
	...
	updates = adam(model.loss, [model.W2, model.b2, model.W3, model.b3], learning_rate) 
...
def main():
	...
	if sys.argv[1] == "pretrain":
		...
		train.run(data, 200, ("pretrain_autoencoder", data['train']))
